...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex h-16 items-center gap-2 p-4", className)}
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-y-2 overflow-auto p-2 group-data-[collapsible=icon]:overflow-hidden",
        className
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center justify-center group-data-[state=expanded]:justify-start gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-accent focus-visible:ring-2 active:bg-accent aria-disabled:pointer-events-none aria-disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 data-[active=true]:bg-background data-[active=true]:font-medium data-[active=true]:text-foreground data-[state=open]:hover:bg-accent group-data-[collapsible=icon]:!size-10 group-data-[collapsible=icon]:p-0 [&>span]:group-data-[state=collapsed]:hidden [&>span]:whitespace-nowrap [&>svg]:size-5 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
...
>(
  (
    ...
    const button = (
      // @ts-expect-error - `Comp` can be a Link, which has an href
      <Comp
        ref={ref}
        href={href}
